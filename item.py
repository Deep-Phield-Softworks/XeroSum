...
-'frame' => Tuple describing frame position in the ItemSheet referenced by
            item_manifest['image_key']
-'weight' => Float value describing the weight of the item
-'use_sound' => Value points to the .ogg file of the sound the item makes on
                use. Default for now is explosion
'''
...
class Item(Matter):
    ...
    def __init__(self, **kwargs):
        ...
        self.accepted_kwargs = {'tall': 0, 'float_offset': plist([0.5, 0.5]),
                                'layer': 1.5, 'frame': (0, 0), 'weight': 0.0,
                                'use_sound': 'explosion.ogg'}
