#!/usr/bin/env python
#Generic image object class
#The purpose of this class is to lower memory overhead. Rather than have
#each figure have it's own surface, there will be one image object for all the
#figures using the same graphics.
#At the startup of the game, a manifest of images should be loaded and each
#image object instantiated.
import pygame, pickle, os, math, datetime, sys
from xeroConstants import *
import libraryXero as lib
class image:
    #Constructor
    def __init__(self, imageName, framesWide, framesHigh, pixelsWide, pixelsHigh): 
        self.imageName = imageName   #Image file name 
        artdir = lib.getArtDir()     #Get os independant Image file path
        imagepath = artdir + self.imageName #Make absolute image path
        ##Use absolute image path to load image with opacity info
        #Make a pygame Surface object to be used directly by this image object
        #and indirectly by all dependant figure objects
        self.surface = pygame.image.load(imagepath).convert()
        ...
        self.pixelsWide = pixelsWide #How many pixels wide is image
        self.pixelsHigh = pixelsHigh #How many pixels high is image
